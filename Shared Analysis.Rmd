...
#### Understanding how and why certain groups are most at risk of dying from opioids is the first step to reducing the magnitude of this epidemic. This information will enable targetted interventions, whether they be at the community or policy level. 
...
Year | 1999 - 2018   
Estimate | Drug overdose death rates     


## Data Limitations

#### While these data do include information on race, gender, and sex, they are not disaggregated to our liking. When Hispanic ethnicity is identified, there is no race to accompany it. This means that we cannot explicitly analyze the influence of ethnicity on a specific race, but rather on all races. For this reason we will create two separate variables relating to race/ethnicity, one that includes race and ethnicity together and another that just specifices Hispanic or not without any racial measure. Additionally, we only have information about sex, since these data are from death certificates. As such, we cannot consider the influence of other potentially relevant factors like one's gender identity. We can also not examine how sexual orientation impacts opioid mortality, as there is no measure of this either. It is important to note that these data are taken from death certificates, but do not include any potentual identifying data. 


## Subsetting the Data

#### These data were organized in a way that required a considerable amount of cleaning and subsetting in order to isolate the measures we intended to explore. Specifically, the demographic measures do not have their own variables, but rather are compiled into one variable, called "STUB_LABEL". We must another variable called "STUB_NAME" and the corresponding numerical variables "STUB_LABEL_NUM" and "STUB_NAME_NUM" to relabel the required variables and subset them accordingly for our analysis. Each "STUB_NAME_NUM" number corresponds to categorical measure(s), for the sake of our analysis, we will focus on the "Sex and race and Hispanic origin" level of the "STUB_NAME" variable which corresponds to 5 in "STUB_NAME_NUM." A similar categorization system exists for specific groups with the "STUB_LABEL" and "STUB_LABEL_NUM" corresponding to "Female: Not Hispanic or Latino: White" and 5.7, respectively. The "STUB_LABEL_NUM" builds upon the "STUB_NAME_NUM" with a decimal number to denote a specific sex, ethnicity, and race. 
...
type_RE <- ggplot(drug_RES, aes(x = factor(Responsible_Drug), y = ESTIMATE, fill = Race_Ethnicity)) +
  ...
  labs(title = "Overdose Estimates by Drug Type and Race/Sex Identity", 
       x = "Drug Type", 
       y = "Estimated Deaths per 100,000") +
  theme_minimal() + theme(axis.text.x = element_text(angle = 90)) + labs(fill = "Race/Sex Identity") + theme(plot.title = element_text(hjust = 0.5)) + ylim(0,20)
