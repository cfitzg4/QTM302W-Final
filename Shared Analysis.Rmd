...
Year | 1999 - 2018   
Estimate | Rate of Overdose Deaths per 100,000 people   
...
In order to answer our research question we need to load in the data and relevant packages that allow us to complete the previously mentioned subsetting and the statistical tests required to compare the estimated death rates between certain groups.
...
We identified how each drug overdose type is categorized so that we can properly relabel it. This allow us to filter accordingly, to illustrate ***how the death rate estimates vary by racial/gender groups based on the type of drug causing the overdose***. Ultimate showing us its impact.
...
2 | drug overdose death from synthetic opioids
3 | drug overdose deaths from methadone
4 | drug overdose deaths from synthetic opioids excluding methadone
...
To clarify the required demographic data: We produced tables and specifically looked for what number is populated by observations which is useful in subsetting the data from the larger data set that is organized by race and sex.

```{r}
##This table tells us which single digit number represents the racial/sex level of the STUB_NAME variable. 

table(drug$STUB_NAME_NUM, drug$STUB_NAME) ## 4 is the number that corresponds to Race and Sex, 

## We then filtered on this to see which decimal number corresponds to each racial/gender group. 
drug_exam <- drug_type %>% 
...
**Conclusion:** After subsetting and cleaning the original dataset, we ended up with 5 variables of of interest.  
1. *"ESTIMATE"* provides the average rate of deaths per 100,000 people.

2. *"Race"* and *"Sex"* show us the racial identity and sex classification provided on the death certificate.

3. *"YEAR"* notes the year in which the deaths were registered.

4. *"Responsible_Drug"* tells us the drug responsible for the death as measured by the death certificate.
